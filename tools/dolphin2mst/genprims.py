...
def main(argv: List[str]) -> int:
    ...
    for cls, items in sorted(by_class.items()):
        ...
        lines = [f'"GENERATED by tools/dolphin2mst/genprims.py from the Dolphin corpus.',
                 ...
                 f"    \"Dolphin writes both `{base} foo` and `{base} default foo`",
                 f"     (the latter at only 10 sites in all of MVP). `default`",
                 f"     answers the class itself so both spellings reach the same",
                 f"     generated methods.\"",
                 f"    {base} class >> default [ ^self ]"]
