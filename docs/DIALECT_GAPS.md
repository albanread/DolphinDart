...
|---|---|---|---|---|---|
| A1 | Source format | UTF-8-BOM CRLF bang-chunk `.cls`; `.pax` manifests carrying pools **and loose methods** | house `.mst`; no chunk reader | **T** | DD3 |
| A2 | Namespaces | namespaces-as-classes, dotted refs, `imports:` | none in the parser | **T** | DD3 |
| A3 | Pools / `##()` / `??` | compile-time folding, ifNil sugar | none | **T** | DD3 |
...
| A5 | Exceptions | ANSI hierarchy, `pass`/`outer`/`retry`/`return:`, **resumable** `Warning`/`Notification` | `on:do:`→`stOnDo`, `ensure:`/`ifCurtailed:`→`stEnsure` (Dart try/catch/finally) **work — 21/21 assertions green (DD0)**; no ANSI class hierarchy in the world | **C+D** | DD4 |
| A6 | Processes | green processes, `ProcessorScheduler`, `Delay`, `Semaphore` | none (house doctrine: no green threads); Dart isolates + a native pump | **C/–** *(census)* | DD8/DD10 |
| A7 | Weak / finalization | weak collections, mourning | none in-image; Dart embedder has weak persistent handles | **–/V** *(census)* | backlog |
| A8 | `become:` | the view-resource proxy | VM has forwarding (`vm/become.h`, hot-reload); ST layer exposes it with guarded refusals | **T** *(census)* | DD3/DD12 |
| A9 | Strings | `Utf16String` at every API edge; `AnsiString`/`Utf8String` | **Dart strings ARE UTF-16** — the native case | **C** *(census)* | DD8 |
...
| A11 | External calls | `<stdcall:>`/`<cdecl:>`, structs, `GetLastError` | per-function C++ natives work; generic `ST_ffiCall` **stubbed on Windows** | **V** | DD6 |
