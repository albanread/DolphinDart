// Demo: Sprite Invaders (WINDART HLSL variant) — the whole game pane: shader sky,
// sprites, SFX, HUD.  IDENTICAL to 13_invaders.dart except the kSky starfield is
// written in HLSL (not MSL) so D3DCompile accepts it and the twinkling starfield
// renders live on Windows (T3). The stock 13_invaders.dart keeps its MSL sky for
// macdart portability; on Windows that MSL body declines gracefully to black.
//
// Every layer of the engine at once, as a playable game. Layer 0: a twinkling
// starfield fragment shader compiled AT RUNTIME from the HLSL string below.
...
// HLSL body for the engine's runtime-shader pane. The engine prepends
// kShaderHeaderHlsl (which declares `cbuffer ShaderUniforms { float time; float
// aspect; ...; float p[8]; }`, the fullscreen `vs_shader`, and `struct GVOut`),
// then compiles this `fmain` at ps_5_0. Same math as the MSL original: MSL
// `fract`->`frac`, `in.uv`->`input.uv`, `u.time/u.aspect`->the cbuffer globals,
// and the scalar `float3(x)` splat spelled out as `float3(x,x,x)`.
const String kSky =
    'float4 fmain(GVOut input) : SV_Target {\n'
    '    float2 uv = input.uv * float2(aspect, 1.0);\n'
    ...
    '        float star = smoothstep(0.14, 0.0, length(cellUv)) * step(0.90, h);\n'
    '        float twinkle = 0.6 + 0.4 * sin(time * (2.0 + h * 4.0) + h * 12.0);\n'
    '        float s = star * twinkle * 1.7;\n'
    '        col += float3(s, s, s * 1.15);\n'   // a touch cooler at the peaks
    '    }\n'
